...
The ``__match_args__`` attribute is always looked up on the result returned
by ``__match__`` (the 'match proxy').  ``__match_args__``, if it is present
and not ``None``, must be a list or tuple of strings naming the allowed
