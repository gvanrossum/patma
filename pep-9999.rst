...
(See Appendix A for the full, unabridged grammar.)
...
* If the missing attribute is present in ``__match_args__`` (which must be a list or tuple
...
* If the ``__match_args__`` attribute is absent on the returned proxy or ``None``,
...
* If the match proxy has a ``__match_args_required__`` attribute (which should
...
looked up on the proxy returned by ``__match__()``.
...
but for _any_ new Python syntax that leverages the capbilities of the PEG parser,
