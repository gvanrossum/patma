...
The named class must be a type (must inherit from ``type``), and cannot be
a function or some other type of callable object.
...
ambiguous. And because of type erasure, generic types such as
``list[int]`` cannot be distinguished as patterns (Python disallows
``isinstance(x, list[int])``), leading to unexpected behavior.
...
The one-off form is only meant to handle a single pattern case, and as such
will not support `if`/`elif` chains::
...
matches - it's better and clearer to use a full match in this case.

Similarly, we don't propose ``while match`` construct present in some languages
