...
``if ... elif ... else`` statement. Also, the recently implemented PEP 617
...
* If an attribute is missing on the proxy, and the class being matched has no ``__match_args__``
  attribute (or it is ``None``)), an ``ImpossibleMatchError`` is raised.
...
* If the missing attribute is present in the class's ``__match_args__`` (which must be a list or tuple
...
* If there are match-by-position items, the item at position ``i`` is matched
  against the value looked up by attribute ``__match_args__[i]``. For example,
  a pattern ``Point2D(5, 8)``, where ``Point2D.__match_args__ == ["x", "y"]``, is translated (approximately) into
  ``obj.x == 5 and obj.y == 8``.
...
* If the ``__match_args__`` attribute is absent on the matched class or ``None``,
  ...
  the exception is also raised. We don't fall back on
...
Special attributes ``__match_args__`` and ``__match_args_required__``
---------------------------------------------------------------------

The ``__match_args__`` and ``__match_args_required__`` attributes are always
looked up on the matched class.

* ``__match_args__``, if it is present and not ``None``, must be a
  list or tuple of strings naming the allowed positional arguments.

* ``__match_args_required__``, if present, gives the minimum number of
  arguments from ``__match_args__`` that must be matched (either by
  position or by keyword).  It must be a nonnegative ``int`` not
  greater than the length of ``__match_args__``.  If the latter is
  absent or ``None``, ``__match_args_required__`` can only be absent,
  ``0`` or ``1``. (The value ``1`` is allowed because if
  ``__match_args__`` is missing or ``None``, a single positional
  argument is allowed which matches the target object itself.)
...
contain various helpers to simply define custom ``__match__()`` methods, in
particular, a thin wrapper class ``MatchWrapper`` that will allow hiding and
adding attributes for match purposes, setting ``__match_args__`` etc.
For example::
...
leading keyword inside  a ``match`` statement, it is easy for a compiler to
...
tedious. Also, there will be actual performance penalty if the user does not
treat pattern object properly.
