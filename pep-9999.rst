...
literals for the purpose of pattern matching.  Similarly, binary addition and
subtraction are allowed in cases where a number had both real and imaginary
parts.
...
* ``["a", *_, "z"]`` matches any sequence that starts with ``"a"`` and ends with
  ``"z"``.
...
For this reason, ``**_`` is invalid in mapping patterns; it is always a no-op
...
* When ``__match_args__`` is missing or ``None`` (as is the default), a single
  ...
  intuitive for most objects: ``bool(False)`` matches ``False`` (but not ``0``),
  and ``tuple([0, 1, 2])`` matches ``(0, 1, 2)`` (but not ``range(3)``).
